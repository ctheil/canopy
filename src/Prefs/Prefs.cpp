...
float Prefs::publishFrequency = 15.0;
std::string Prefs::deviceId = "untitled_dev_board"; // default

void Prefs::syncWithFlash()
{
}
void Prefs::setup()
{
  flashPreferences.begin("canopy", false);
  std::string setId = std::string(flashPreferences.getString(pkeys.deviceId).c_str());
  if (setId != "")
  ...
  Comms::addTopic(Comms::endpoint("/preferences/put-device-id"), [&](std::string payload)
                  { flashPreferences.putString(pkeys.deviceId, payload.c_str()); Prefs::deviceId = payload; Prefs::publishState(); }); // WILL PUBLISH TO UPDATED DEVICE ID ENDPOINT... FIXME?
}
...
void Prefs::publishState()
{
  Comms::mqttClient.publish(
      ...
      0,
      true, // retain
      std::string(std::to_string(Prefs::publishFrequency) + "," + Prefs::deviceId).c_str());
}